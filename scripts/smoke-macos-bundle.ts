...
export async function smokeMacOSBundle(appPath: string, timeoutMs = 20_000): Promise<void> {
  ...
  const root = mkdtempSync(join(tmpdir(), "homebrain-smoke-"));
  ...
  const executable = join(installed, "Contents", "MacOS", "homebrain");
  const child = Bun.spawn([executable, "serve"], {
    ...
    env: {
      ...process.env,
      HOME: home,
      HOMEBRAIN_DATA_DIR: join(home, "Library", "Application Support", "Homebrain"),
      HOMEBRAIN_LOG_DIR: join(home, "Library", "Logs", "Homebrain"),
      HOMEBRAIN_WEB_HOST: "127.0.0.1",
      HOMEBRAIN_WEB_PORT: String(port),
      HOMEBRAIN_SERVICE_MANAGED: "0",
      HOMEBRAIN_CLAUDE_BIN: join(root, "missing-claude"),
      HOMEBRAIN_TRAE_BIN: join(root, "missing-trae"),
    },
  });
  try {
    try {
      ...
      if (!body.includes("homebrain") || !body.includes("设置进度")) {
...
if (import.meta.main) {
  try {
    ...
    const app = at >= 0 ? process.argv[at + 1] : join(import.meta.dir, "..", "dist", "Homebrain.app");
