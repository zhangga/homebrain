...
Homebrain is licensed under Apache License 2.0. A release bundle must include this file and the
...
- Use in Homebrain: separately bundled runtime executing Homebrain's JavaScript bundle
...
Bun's license inventory. Homebrain keeps its application JavaScript separate from that executable.
...
- Use in Homebrain: embedded application dependency
...
- Use in Homebrain: separately bundled, unmodified Feishu/Lark command-line executable
...
- Distribution policy: not included in `Homebrain.app` or its DMG

Homebrain may offer an explicit-consent installer that downloads an official Codex release directly
...
Homebrain's attachment helper links Apple Vision and PDFKit system frameworks. These frameworks are
provided by macOS and are not redistributed in the Homebrain release.
