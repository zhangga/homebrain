...
- **飞书连接**：首次连接使用引导式设置，一键创建飞书应用并自动识别 Bot 名称与 open_id；也保留
  ...
  智能应答模式，并直接发送测试消息验证发送通道。对外共享状态按 App ID 独立记录，更换机器人后不会沿用
  旧机器人的验证结果。
...
给群指定 Agent 后，回答和群聊参与判断都使用该 Agent 的 CLI。默认智能应答模式下，被 @ 时一定回答；
未 @ 的消息会先由模型判断是否为面向群体的真实提问，只有值得参与时才主动回答，普通聊天继续静默收录。
关闭智能应答开关后，群内任意消息都会应答；`Topic reply` 控制是否在话题内回复。
...
群内非 @ 消息的静默收录和智能应答依赖敏感权限 `im:message.group_msg`，该权限已经放进首次创建确认页。
如果管理员在创建阶段未批准，智能应答只能处理 @ 消息，
关闭智能应答开关也无法让飞书把其他群消息推送给机器人。
