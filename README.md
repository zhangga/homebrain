...
- **学习**（引导式每日阅读）：先把书籍附件或飞书文档发给机器人，再回复原消息发送 `/learn new <书名>`。HomeAgent 会保存清洗后的原文快照，按标题和段落边界每天截取一课，只依据当日原文生成导读与 2–3 个思考题。
  - 用 `学习回答：<你的回答>` 提交作答；Agent 会给出点评、澄清、总结与下一步。只有回答成功或显式跳过后才推进阅读游标，完成记录会以 `source=learning` 写回知识空间。
  - `/learn` 查看计划；`/learn pause <名称或序号>`、`/learn resume <名称或序号>`、`/learn skip <名称或序号>`、`/learn delete <名称或序号>` 分别暂停、恢复、跳过当前课和删除计划。
  - 默认每天北京时间 8:00 推送，停机后启动会补发当日未送课程；发送失败保留同一课重试，不会误推进。管理后台可查看进度、课程记录，并调整推送时间与每课字数。
  - 回复原书执行「别记这条」会同时删除派生的学习快照与课程历史；计划、原文快照和历史也随空间导出、恢复或删除。
...
- **数据治理**：按空间导出 `homeagent.space v2` JSON 完整备份（知识页、原始记录、撤回标记、任务、提醒、学习计划及原文快照、空间元数据及关联 Agent），恢复 v1/v2 备份，或永久删除整个空间；可按保留周期立即清理已提炼的过期消息。
...
后续已完成：研究任务、引导式每日阅读学习计划、真实飞书 E2E、思考表情、精确消息撤回、健康检查与可观测性（`/healthz`、`/readyz`、运行状态页与异常提示）、空间导出/恢复/删除、原始消息保留策略、非本机后台鉴权、P2 首版附件提炼、P3.1 飞书配置向导、P3.2 macOS LaunchAgent 后台常驻与服务管理，以及 P4.1 CLI-only 意图路由。
